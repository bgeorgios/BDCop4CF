...
# compile Stan model
lgp.model <- stan_model("dynamicGumbel.stan")

# read data
data <- read.csv("Washington-Q-S.csv")
...
a.init <- list(b = 3.0, w = 0.05)
b.init <- list(b = 2.5, w = 0.15)
c.init <- list(b = 5.0, w = -0.10)
d.init <- list(b = 4.0, w = -0.20)
...
logLikelihood <- function(theta) {
  ...
  for (j in 1:nrow(pseudo.data)) {
    copu <- gumbelCopula(param = b + w * x[j])
    ll <-
